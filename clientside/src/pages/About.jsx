    ...
    <div className="bg-[#0d2b0d] min-h-screen">
      ...
      <section className="py-10 px-4">
        <div className="max-w-6xl mx-auto grid md:grid-cols-2 gap-8 items-center">
          <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} className="bg-[#1a3a1a]/60 rounded-xl p-6 border border-green-500/20">
            <h2 className="text-xl font-bold mb-4 text-green-300">Who We Are</h2>
          ...
          <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} className="bg-gradient-to-br from-green-700 to-green-800 rounded-xl p-6 text-center">
            <div className="grid grid-cols-2 gap-6">
              <div>
                <div className="text-3xl font-bold text-white mb-1">500+</div>
                <div className="text-green-200 text-xs">Projects</div>
              </div>
              <div>
                <div className="text-3xl font-bold text-white mb-1">150+</div>
                <div className="text-green-200 text-xs">Clients</div>
              </div>
              <div>
                <div className="text-3xl font-bold text-white mb-1">50+</div>
                <div className="text-green-200 text-xs">Experts</div>
              </div>
              <div>
      ...
      <section className="py-10 px-4">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-8">
            <h2 className="text-2xl font-bold mb-2 text-white">Our Core Values</h2>
            <p className="text-gray-400 text-sm">The principles that guide everything we do</p>
          </div>
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-5">
            {values.map((val, idx) => (
              <motion.div key={idx} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} className="text-center p-5 bg-[#1a3a1a]/60 rounded-xl border border-green-500/20 hover:border-green-500/40 transition">
                <div className="flex justify-center mb-3 text-3xl text-green-400">{val.icon}</div>
                <h3 className="text-sm font-semibold text-white mb-2">{val.title}</h3>
      ...
      <section className="py-10 px-4 bg-[#0a2a0a]/30">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-8">
            <h2 className="text-2xl font-bold mb-2 text-white">Our Journey</h2>
            <p className="text-gray-400 text-sm">Milestones that define our growth</p>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
            {milestones.map((milestone, idx) => (
              <motion.div key={idx} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.1 }} className="text-center p-3 bg-[#1a3a1a]/60 rounded-lg border border-green-500/20">
                <div className="text-green-400 font-bold text-lg">{milestone.year}</div>
                <div className="text-white text-xs font-semibold mt-1">{milestone.title}</div>
                <div className="text-gray-500 text-[10px] mt-1">{milestone.desc}</div>
              </motion.div>
            ))}
          </div>
        </div>
